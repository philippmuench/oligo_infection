...
for (cat in (unique(YL58_orig$Subcategory))){
  ...
  if(test$p.value <0.05){
    ...
    pdf(paste0("analysis7/sig_heatmap_subsystem_", cat,".pdf"),width = 15, height=10)
    print(p)
    dev.off()
    
  } else {
    pdf(paste0("analysis7/heatmap_subsystem_", cat,".pdf"), width = 15, height=10)
    print(p)
    dev.off()
  }
...
write.table(pval_mat, file="analysis7/pval_subcategory.txt", sep='\t', row.names = F, quote = F)